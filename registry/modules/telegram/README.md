...
```typescript
axon.hooks.hook("telegram:message", async ({ text, chatId, username, reply, replyMarkdown, replyWithButtons }) => {
...
```typescript
axon.hooks.hook("telegram:command", async ({ command, args, reply }) => {
    switch (command) {
        case "review":
...
```typescript
axon.hooks.hook("telegram:button", async ({ data, answer, reply }) => {
...
```typescript
axon.hooks.hook("telegram:message", async ({ text, chatId, reply }) => {
    await telegram.typing(chatId)
