 ...
 */
async function agentWithModule(platform: PlatformT, dir: string) {
    ...
    await writeFile(
        ...
        `export default defineAgent({ providers: [Mock()], model: "mock:mock", modules: ["./modules/broken"] })\n`,
    )
    return { project, moduleTool }
}

async function withAgent(
    run: (platform: PlatformT, project: Awaited<ReturnType<typeof mockAgent>>) => Promise<void>,
): Promise<void> {
    const storeDir = await mkdtemp(join(tmpdir(), "axon-test-store-"))
    const dir = await mkdtemp(join(tmpdir(), "axon-test-dir-"))

    // Seeded through a FIRST platform, before the one under test is built:
    // Cloud() reads the active profile once when it constructs its client, so
    // a profile saved onto a live platform lands on disk but never reaches the
    // client already built from it. These tests INVOKE the agent, and
    // inference is the supervisor's — an agent holds no key and reaches it
    // over the link — so without this a wake fails with "no Authorization
    // header" rather than the reload failing.
    const seed = supervised(storeDir)
...
async function withModuleAgent(
    ...
    const storeDir = await mkdtemp(join(tmpdir(), "axon-test-store-"))
    const dir = await mkdtemp(join(tmpdir(), "axon-test-dir-"))
    // Authenticated like withAgent above: spawning binds inference, which
    // resolves against the ACTIVE profile — so an agent booted with nobody
    // logged in falls through to the default (metered) pool.
...
describe("agent reload", () => {
    ...
    it("drops a field the author removed — a reload is the whole config re-read, not a merge", async () => {
        await withAgent(async (platform, project) => {
            // `links` rather than `description`: the latter is deprecated and
            // commented out of AxonConfig (package.json carries it now), so
            // this asserted on a field that no longer exists. The SUBJECT is
            // unchanged — a reload re-reads the whole config rather than
            // merging over the old one — and any optional field the author can
            // delete demonstrates it.
            await writeFile(
                join(project.root, "axon.config.ts"),
                `export default defineAgent({ providers: [Mock()], model: "mock:mock", links: { docs: "/docs" } })\n`,
            )
            await platform.agents.spawn(project)
            const agent = platform.agents.current!
            expect(platform.agents.blueprint!.config.links).toEqual({ docs: "/docs" })

            await writeFile(join(project.root, "axon.config.ts"), `export default defineAgent({ providers: [Mock()], model: "mock:mock" })\n`)
            await agent.reload()

            expect(platform.agents.blueprint!.config.links).toBeUndefined()
        })
    }, 90_000)
    ...
    it("survives a failed reload — the previous blueprint stays live", async () => {
        await withAgent(async (platform, project) => {
            ...
            expect(platform.agents.session!.id).toBe(sessionId)
            await expect(ask(agent, "after the bad edit")).resolves.toBeDefined()
        })
    }, 90_000)
})
