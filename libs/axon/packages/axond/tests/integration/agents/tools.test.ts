...
async function withAgent(
    ...
    // a profile saved onto a live platform never reaches the client already
    // built from it. These tests INVOKE the agent, and inference is the
    // supervisor's — an agent holds no key and reaches it over the link — so
    // without this a wake fails with "no Authorization header".
