...
import { supervised } from "../../setup/supervised"
...
/**
 * Write the profile to disk, then hand back a platform built FROM it.
 *
 * Writing onto a live platform is enough for `profiles.active()` (which reads
 * disk per call) and NOT enough for the cloud client: Cloud() reads the active
 * profile once, when it builds its client. A test that only lists sessions
 * never noticed; one that INVOKES an agent fails its wake with "no
 * Authorization header", because inference is the supervisor's and it has no
 * credential.
 *
 * So the write happens through a throwaway platform and the returned one is
 * constructed afterwards, which is the only ordering that satisfies both.
 */
function login(storeDir: string): PlatformT {
    const seed = supervised(storeDir)
    seed.store.profiles.save(TEST_USER.id, {
        user: { id: TEST_USER.id, email: TEST_USER.email },
        auth: { apiKey: TEST_USER.apiKey },
    })
    return supervised(storeDir)
}

/** Create a named agent under the ACTIVE profile — sessions() only scans there. */
async function profileAgent(platform: PlatformT, name: string) {
