 ...
 *
 * The twin of libs/axon/platform/tests/setup/preload.ts. Both suites boot
 * agents against the same local staging stack and need the same guarantees
 * (daemon up, no ambient API key, a funded TEST_USER), so the setup is the
 * same setup. It is duplicated rather than imported because a preload is
 * wired by PATH in bunfig.toml and reaches its own package's fixtures —
 * exporting one package's test scaffolding to another would make it a
 * published surface.
 *
