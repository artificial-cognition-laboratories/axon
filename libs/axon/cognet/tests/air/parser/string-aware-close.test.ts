...
describe("Air parser: string-aware close-tag scanning", () => {
    it("does not close early on a closing tag written as a double-quoted string literal", () => {
        ...
        const code = `const s = "</typescript>"`

        const events = parser.feed(`<typescript>${code}</typescript>`)

        expect(events).toContainEqual({ type: "typescript:done", content: code })
    })
    ...
    it("respects escaped quotes inside a string when scanning for the real close tag", () => {
        const parser = Air().parser()
        const code = `const s = "he said \\"</typescript>\\" once"`
    ...
    it("applies string-aware scanning to shell blocks too", () => {
        const parser = Air({ modes: [{ type: "shell" }] }).parser()
        const code = `echo "</shell>"`

        const events = parser.feed(`<shell>${code}</shell>`)

        expect(events).toContainEqual({ type: "shell:done", content: code })
    })
