...
export type AxonHandle = {
    ...
    /**
     * Minimal session identity — enough for a script to key its own state
     * or logging off which session it's running in. Everything else about
     * the session (log, kernelLog, raw commit) is core-internal.
     */
    session: { id: string }
}
