...
export function SandboxProcs(opts: ProcsOpts) {
    ...
    async function mediateAndSpawn(entry: ProcEntry, command: string, spawnOpts?: ProcSpawnOptions): Promise<void> {
        ...
        if (killedBeforeSpawn.has(procId)) {
            ...
            wire.emit("capsule:proc:denied", { procId, command, error: "killed before it could spawn" })
            markExited(entry, -1)
            return
        }

        if (!allowed) {
            wire.emit("capsule:proc:denied", { procId, command, error: "denied by policy" })
            markExited(entry, -1)
            return
        }
        ...
        if (child.pid === undefined) {
            wire.emit("capsule:proc:denied", { procId, command, error: "spawn failed" })
            markExited(entry, -1)
            return
        }
        ...
        wire.emit("capsule:proc:spawned", { procId, pid: child.pid, command, cwd: entry.cwd, kind: "managed" })
        ...
        child.on("exit", code => {
            ...
            wire.emit("capsule:proc:exit", { procId, code: code ?? -1, durationMs: Date.now() - entry.startedAt })
        })
    }
    ...
    async function run(command: string, runOpts?: ProcRunOptions, signal?: AbortSignal): Promise<ProcRunResult> {
        ...
        return new Promise<ProcRunResult>((resolve, reject) => {
            ...
            wire.emit("capsule:proc:spawned", {
                procId,
                pid: child.pid,
                command,
            ...
            child.on("error", err => {
                ...
                wire.emit("capsule:proc:exit", { procId, code: -1, durationMs: Date.now() - startedAt })
                settle(() => resolve({ ok: false, exitCode: -1, stdout, stderr, err: err.message }))
            })
            child.on("exit", code => {
                runChildren.delete(procId)
                wire.emit("capsule:proc:exit", { procId, code: code ?? -1, durationMs: Date.now() - startedAt })
    ...
    function stdin(procId: string, data: string): void {
        ...
        if (!child?.stdin?.writable) {
            wire.emit("capsule:proc:stdin:error", { procId, error: "process not running or stdin closed" })
            return
        }
