...
export function Scope(opts: ScopeOpts) {
    ...
    async function loadTool(tool: Extract<CapsuleCommand, { type: "tool:load" }>): Promise<void> {
        try {
            ...
            wire.emit("capsule:tool:loaded", { namespace: tool.namespace, fns: Object.keys(values) })
        } catch (err) {
            wire.emit("capsule:tool:error", { namespace: tool.namespace, error: err instanceof Error ? err.message : String(err) })
        }
    }
