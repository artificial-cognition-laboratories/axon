...
export function CapsuleSubprocess(opts: SubprocessOpts) {
    ...
    function hardInterrupt(): Promise<void> {
        ...
        hardReset = (async () => {
            ...
            try {
                ...
                bus.emit("capsule:dead", { reason: err instanceof Error ? err.message : String(err) })
                throw err
            }
    ...
    bus.on("capsule:exit", () => {
        ...
        if (restartCount > max) {
            bus.emit("capsule:dead", { reason: "max restarts exceeded" })
            return
        }

        bus.emit("capsule:restarting", { restartCount })
        Build({ config, bus })
            .then(runtime => {
                current = runtime
                bootedAt = Date.now()
                bus.emit("capsule:restarted", { restartCount })
            })
            .catch(err => {
                bus.emit("capsule:dead", { reason: err instanceof Error ? err.message : String(err) })
            })
