...
export function Wire(opts: WireOpts): WireT {
    ...
    function handleLine(line: string) {
        ...
        try {
            ...
            try {
                ...
                if (recovery.length > RECOVERY_BUFFER_MAX) {
                    bus.emit("capsule:parse:error", { reason: "recovery buffer exceeded 1MB, discarding" })
                    recovery = ""
                }
                return
            }
        ...
        if (parsed === null || typeof parsed !== "object") {
            bus.emit("capsule:parse:error", { reason: "non-object protocol line", line })
            return
        }

        if (!isCapsuleEvent(parsed)) {
            bus.emit("capsule:parse:error", { reason: "invalid event shape", line })
            return
        }
