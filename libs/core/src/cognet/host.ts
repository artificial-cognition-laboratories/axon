...
import type { AxonBlueprint, AxonRunResult, CognetConfig, CognetDefinition, CognetHooks, CognetPlugin, CognetWake, KernelAbi } from "@arcforge/types"
 ...
 *                      system, blueprint) and normal imports.
 *
 ...
 * - process-lifetime things are ambient (kernel, blueprint, loop)
...
let boundBlueprint: AxonBlueprint | null = null
let currentEcs: ReturnType<typeof Ecs> | null = null
let loaded = false
...
type CognetAmbientScope = {
    ...
    readonly blueprint: AxonBlueprint
}
...
function ecsOrThrow(): ReturnType<typeof Ecs> {
    if (!currentEcs) throw err("COGNET_ACCESSED_BEFORE_LOAD", { detail: "phase()/system() are wake-scoped — call them inside the loop body" })
    return currentEcs
}
...
const localScope: CognetAmbientScope = {
    kernel: localKernel,
    loop: registerLoop,
    phase: <T>(name: string, fn: () => Promise<T>) => ecsOrThrow().phase(name, fn),
    system: <T>(name: string, fn: () => Promise<T>) => ecsOrThrow().system(name, fn),
    get blueprint() {
        if (!boundBlueprint) throw err("COGNET_ACCESSED_BEFORE_LOAD", { detail: "blueprint is not available yet — it binds before load()" })
...
Object.defineProperty(globals, "blueprint", {
    configurable: true,
    get() {
        return ambientOrThrow().blueprint
    },
})
...
export function CognetHost(config: CognetConfig, main: () => Promise<void>): CognetDefinition {
    return {
        ...
        async wake(wake) {
            ...
            const ecs = Ecs({ emit: (type, data) => kernelOrThrow().emit(type, data), signal: wake.signal })
            currentEcs = ecs
            ...
            try {
                ...
                while (!stopped && !wake.signal.aborted) {
                    ...
                    await ecs.tick(async () => {
                        await hooks.callHook("tick", { tick: ecs.state.tick })
                        await body(ctx)
                    })
                }
            } finally {
