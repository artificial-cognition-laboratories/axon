...
export async function loadModule(module: AxonModule): Promise<LoadedModule> {
    ...
    const options = validateOptions(module, config)
 ...
 * declared defaults. The schema on the blueprint (AST-scraped at scan time) is
 * the validation authority — the imported config is used for behaviour only,
 * never re-read for options, so the two can never disagree.
 */
function validateOptions(module: AxonModule, _config: ModuleConfig): Record<string, unknown> {
