...
import { Air, type AirBlockEvent } from "../platform/air"
...
export function Engine(opts: EngineOpts) {
    ...
    async function* stream(req: AxonEngineRequest, context: EngineContext): AsyncGenerator<AxonEngineEvent> {
        ...
        if (!current) {
            ...
            await opts.session.commit("kernel:engine:failed", { attempts: 0, fault }, span)
            ...
            try {
                ...
                if (canRetry) {
                    ...
                    try {
                        ...
                        await opts.session.commit("kernel:engine:failed", { attempts: attempt, fault: abortFault }, span)
                ...
                await opts.session.commit("kernel:engine:failed", { attempts: attempt, fault }, span)
